...
export function Home({ lessons }: Props) {
  ...
  return (
      <main className="app-scrollbar flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top_right,rgba(59,130,246,0.16),transparent_28%),linear-gradient(180deg,rgba(9,9,11,0.92),rgba(9,9,11,0.98))]">
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.85fr)]">
          <div className="rounded-[30px] border border-white/10 bg-zinc-950/85 p-6 shadow-2xl shadow-black/25 lg:p-8">
            <div className="mb-6 flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-blue-400/30 bg-blue-500/10 px-3 py-1 text-[11px] uppercase tracking-[0.14em] text-blue-200">{t("ui.learnByDoing")}</span>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3">
              {firstLesson && <Link to={`/lesson/${nextLesson?.id ?? firstLesson.id}`} className="rounded-2xl bg-blue-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-400">{t("ui.startLearning")}</Link>}
          ...
          <div className="rounded-[30px] border border-white/10 bg-[#0d1117] p-5 shadow-2xl shadow-black/20">
            ...
            <div className="space-y-3 font-mono text-sm leading-7 text-zinc-300">
              <div><span className="text-cyan-300">$</span> python main.py</div>
              <div className="text-zinc-500">{t("ui.previewStatus")}</div>
              <div className="text-blue-200">{t("ui.homeTip")}</div>
            </div>
          </div>
        </section>
          ...
          <div className="rounded-[28px] border border-white/10 bg-zinc-950/80 p-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {lessons.map((lesson, index) => {
                ...
                return (
                  <Link key={lesson.id} to={`/lesson/${lesson.id}`} className="rounded-[22px] border border-white/10 bg-white/[0.04] p-4 transition hover:border-blue-400/35 hover:bg-blue-500/8">
            ...
            <div className="space-y-3">
              {[t("ui.howItWorks1"), t("ui.howItWorks2"), t("ui.howItWorks3")].map((item, index) => (
                <div key={item} className="flex gap-4 rounded-[22px] border border-white/10 bg-white/[0.04] p-4">
                  <div className="grid h-9 w-9 shrink-0 place-items-center rounded-2xl bg-blue-500/10 font-mono text-sm text-blue-200">{index + 1}</div>
