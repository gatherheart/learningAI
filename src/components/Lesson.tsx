      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 lg:px-6 lg:py-6">
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)]">
          <div className="rounded-[30px] border border-white/10 bg-zinc-950/85 p-6 shadow-2xl shadow-black/25">
            <div className="mb-5 flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-blue-400/30 bg-blue-500/10 px-3 py-1 text-[11px] uppercase tracking-[0.14em] text-blue-200">lesson {String(lessonIndex + 1).padStart(2, "0")}</span>
          ...
          <div className="rounded-[30px] border border-white/10 bg-[#0d1117] p-5 shadow-2xl shadow-black/20">
            ...
            <div className="mb-4 h-2 overflow-hidden rounded-full bg-white/10">
              <div className="h-full rounded-full bg-gradient-to-r from-blue-500 via-sky-400 to-cyan-300 transition-all" style={{ width: `${Math.round((progress.completed / progress.total) * 100) || 0}%` }} />
            </div>
            <div className="space-y-3 font-mono text-sm leading-7 text-zinc-300">
              <div><span className="text-cyan-300">$</span> {lesson.bin}</div>
              ...
              <div className="text-blue-200">Open the external workspace if you want to run or extend the code outside this lesson page.</div>
              <div className={progress.done ? "text-cyan-300" : "text-blue-200"}>{progress.done ? "status: unlocked next lesson" : "status: quizzes still pending"}</div>
              <div className="text-zinc-500">keyboard: left/right arrows for lesson navigation</div>
            </div>
            {allDone && <div className="mt-5 rounded-[22px] border border-cyan-400/25 bg-cyan-500/10 p-4 text-sm text-cyan-100">{t("ui.courseComplete")}</div>}
          </div>
        </section>
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
          ...
          <div className="space-y-4">
            ...
            {progress.done && <div className="rounded-[24px] border border-cyan-400/25 bg-cyan-500/10 px-4 py-3 text-sm text-cyan-100">{t("ui.lessonComplete")}</div>}
